...
def pressure_loss_from_pipe(pipe_diameter, pipe_length,
    ...
    pressure_loss = (-friction_factor * pipe_length * 998.2 * fluid_velocity * fluid_velocity) / (2000 * pipe_diameter)
    return round(pressure_loss, 3)

def pressure_loss_from_fittings(
        fluid_velocity, quantity_fittings):
    pressure_loss = (-.04 * 998.2 * fluid_velocity * fluid_velocity * quantity_fittings) / 2000
    return round(pressure_loss, 3)

def reynolds_number(hydraulic_diameter, fluid_velocity):
    reynold = (998.2 * hydraulic_diameter * fluid_velocity) / 0.0010016
    return round(reynold)

def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number, smaller_diameter):
    constant =(0.1 + (50/reynolds_number)) * ((larger_diameter/smaller_diameter)**4 - 1)
    lost_pressure = (-constant * 998.2 * fluid_velocity * fluid_velocity) / 2000
